...
export default function ProfileLink(props: ProfileLinkProps) {
    ...
    const onClick = async () => {
        if (props.isLogoutBttn && !props.screenName) {
        ...
        if (props.screenName) {
            navigation.navigate('Stack', { screen: `${props.screenName}` });
        } else if(props.externalLink){
            AlertExternalLinking()
        }    
        else {
            Alert.alert('Página não encontrada', 'Ainda não existe uma tela para \n\n"' + props.label + '"')
        }
    }

    return (
        <TouchableOpacity activeOpacity={0.6} style={styles.areaLink} onPress={() => onClick()}>
            <Feather style={styles.icon} name={props.iconName} size={18} color={'#8b8b8b'} />
